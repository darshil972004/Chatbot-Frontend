...
/**
 * Update a quick reply for an agent
 */
export async function updateAgentQuickReply(
  agentId: number | string,
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Failed to update quick reply (${res.status}): ${text || res.statusText}`);
  }

  const response = await res.json();
  if (response?.success === false) {
    const message = response?.error?.message || 'Failed to update quick reply';
    throw new Error(message);
  }
